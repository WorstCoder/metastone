...
public class SynergyGameLogic extends GameLogic {
	...
	protected void resolveBattlecry(int playerId, Actor actor) {
		...
		if (hasAttribute(player, Attribute.DOUBLE_BATTLECRIES) && actor.getSourceCard().hasAttribute(Attribute.BATTLECRY)) {
			...
			List<SpellDesc> battlecrySpells = getPossibilities(battlecrySpellDesc, context, player, battlecrySpellDesc, actor, null);
	...
	public void castSpell(int playerId, SpellDesc spellDesc, EntityReference sourceReference, EntityReference targetReference,
						  boolean childSpell) {
		if (spellDesc.getSpellClass() == JoustSpell.class) {

			List<SpellDesc> spells = joustChances(spellDesc);
			SynergyGameContext[] clones = new SynergyGameContext[spells.size()];
			for(int i=0; i<spells.size(); i++){
				clones[i] = ((SynergyGameContext)context).clone();
				clones[i].getLogic().castSpell(playerId,spells.get(i),sourceReference,targetReference,childSpell);
			}
		}
		Player player = context.getPlayer(playerId);
		Entity source = null;
		...
		try {
			...
			if (spellDesc.contains(SpellArg.POSSIBILITY)
					&& spellDesc.contains(SpellArg.RANDOM_TARGET)
					&& spellDesc.contains(SpellArg.TARGET)
					//&& spellDesc.get(SpellArg.TARGET)==null
					&& targets != null
					&& !spellDesc.getSpellClass().equals(MissilesSpellC.class)) {
				targets.clear();
	...
	protected List<SpellDesc> getPossibilities(SpellDesc spell, GameContext context, Player player, SpellDesc desc, Entity source, List<Entity> targets) {
		...
		if (spell.getSpellClass() == JoustSpell.class) {
			return joustChances(spell);
		}
		possibleSpells.add(spell);
		if (isRandom(spell)) {
			possibleSpells.clear();
			Spell unrandomizedSpell = getUnrandomizedSpell(spell);
			List<Object> possibilities = unrandomizedSpell.getPossibilities((SynergyGameContext) context, player, spell, source, targets);
			logger.info("Card: " + source.getName() + " Possibilities: " + (possibilities != null ? possibilities.size() : 0));
			if (possibilities == null) {
				SpellDesc newSpell = spell.clone();
	...
	private List<SpellDesc> metaPossibilities(SpellDesc root, GameContext context, Player player, SpellDesc desc, Entity source) {
		...
		for (SpellDesc spell : rootSpells) {
			...
			if (isRandom(spell)) {
				Spell unrandomizedSpell = getUnrandomizedSpell(spell);
				List<SpellDesc> innerPossSpells = getPossibilities(spell, context, player, desc, source, null);
	...
	private Spell getUnrandomizedSpell(SpellDesc spell) {
		...
		if (spellClass == RandomlyCastSpell.class) {
			return new RandomlyCastSpellC();
		}
		if (spellClass == EitherOrSpell.class) {
			return new EitherOrSpellC();
		}

		if (spellClass == MissilesSpell.class) {
			return new MissilesSpellC();
		}
	...
	private boolean isRandom(SpellDesc spell) {
		...
		if (spell.contains(SpellArg.RANDOM_TARGET)
				&& spell.contains(SpellArg.TARGET)
				&& spell.getSpellClass() != MissilesSpellC.class
				&& spell.getSpellClass() != MissilesSpell.class)
			return true;

		if (spell.contains(SpellArg.VALUE)
				&& spell.get(SpellArg.VALUE).getClass() == RandomValueProvider.class
				|| spell.contains(SpellArg.ATTACK_BONUS)
				&& spell.get(SpellArg.ATTACK_BONUS).getClass() == RandomValueProvider.class)
			return true;
		...
		if (spellClass == PutRandomMinionOnBoardSpell.class
				&& spell.get(SpellArg.CARD_LOCATION) == CardLocation.HAND) {
			return true;
		}
		if (spellClass == PutRandomSecretIntoPlaySpell.class) {
			return true;
		...
		if (spellClass == EitherOrSpell.class
				&& spell.get(SpellArg.CONDITION).getClass() == RandomCondition.class) {
			return true;
		}

		if (spellClass == MissilesSpell.class
				&& spell.get(SpellArg.HOW_MANY) == (Object) 1
				&& spell.get(SpellArg.VALUE) != (Object) 1) {
			return true;
		}
	...
	public List<SpellDesc> joustChances(SpellDesc desc) {
